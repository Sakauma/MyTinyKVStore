...
std::string trend_summary_to_json(const TrendSummary& summary) {
    std::ostringstream out;
    out << '{'
        << "\"count\":" << summary.count
...
int run_benchmark_trend_json_entrypoint(const std::string& directory_path, size_t recent_window) {
    std::cout << trend_summary_to_json(collect_benchmark_trend_summary(directory_path, recent_window)) << std::endl;
    return 0;
}
