...
FormatInspectResult capture_inspect_format(const std::string& db_path, int expected_status = 0) {
    const FormatInspectResult result = inspect_format_analysis(db_path);
    require(result.status == expected_status, "inspect-format should return the expected status");
    return result;
}

FormatVerifyResult capture_verify_format(const std::string& db_path) {
    return verify_format_analysis(db_path);
}
...
FormatInspectResult inspect_format_analysis(const std::string& db_path) {
    ...
    if (snapshot.gcount() != static_cast<std::streamsize>(sizeof(snapshot_header))) {
        return {2, "snapshot_exists=1 snapshot_truncated=1"};
    }
    ...
    if (wal.gcount() == 0) {
        out << " wal_exists=1 wal_empty=1 wal_size=" << file_size_or_zero(wal_path)
            << " wal_records=0"
            << " wal_put_records=0"
            << " wal_delete_records=0"
            << " wal_int_keys=0"
    ...
    if (wal.gcount() != static_cast<std::streamsize>(sizeof(wal_header))) {
        out << " wal_exists=1 wal_truncated=1 wal_size=" << file_size_or_zero(wal_path)
            << " wal_records=0"
            << " wal_put_records=0"
            << " wal_delete_records=0"
            << " wal_int_keys=0"
...
FormatVerifyResult verify_format_analysis(const std::string& db_path) {
    const FormatInspectResult inspect = inspect_format_analysis(db_path);
    if (inspect.status != 0) {
        return {inspect.status, inspect.output_line + " verify_reason=inspect_error", "inspect_error"};
    }

    const auto values = parse_kv_line(inspect.output_line);
    ...
    return {status, inspect.output_line + " verify_reason=" + verify_reason, verify_reason};
}
...
int compatibility_matrix_analysis() {
    ...
    const auto current_inspect_values = parse_kv_line(current_inspect.output_line);
    const auto current_verify = capture_verify_format(current_db_path);
    const auto current_verify_values = parse_kv_line(current_verify.output_line);
    require(current_verify.status == 0, "current compatibility matrix case should verify cleanly");
    require(current_inspect_values.at("snapshot_version") == "2", "current case should report snapshot v2");
    require(current_inspect_values.at("rewrite_recommended") == "0", "current case should not recommend rewrite");
    ...
    const auto live_wal_inspect_values = parse_kv_line(live_wal_inspect.output_line);
    const auto live_wal_verify = capture_verify_format(live_wal_db_path);
    const auto live_wal_verify_values = parse_kv_line(live_wal_verify.output_line);
    require(live_wal_verify.status == 0, "current v2 layout with live WAL should verify cleanly");
    require(live_wal_inspect_values.at("snapshot_version") == "2", "live WAL case should keep snapshot v2");
    require(live_wal_inspect_values.at("wal_version") == "2", "live WAL case should report WAL v2");
    require(live_wal_inspect_values.at("wal_records") == "2", "live WAL case should retain post-snapshot WAL records");
    require(live_wal_inspect_values.at("rewrite_recommended") == "0",
    ...
    const auto truncated_inspect_values = parse_kv_line(truncated_inspect.output_line);
    const auto truncated_verify = capture_verify_format(truncated_db_path);
    const auto truncated_verify_values = parse_kv_line(truncated_verify.output_line);
    require(truncated_verify.status == 2, "truncated WAL case should require rewrite");
    require(truncated_inspect_values.at("snapshot_version") == "2", "truncated WAL case should keep snapshot v2");
    require(truncated_inspect_values.at("wal_truncated") == "1", "truncated WAL case should report WAL truncation");
    require(truncated_inspect_values.at("rewrite_recommended") == "1",
    ...
    require(rewrite_format_analysis(truncated_db_path) == 0,
    ...
    const auto rewritten_truncated_inspect_values = parse_kv_line(rewritten_truncated_inspect.output_line);
    const auto rewritten_truncated_verify = capture_verify_format(truncated_db_path);
    const auto rewritten_truncated_verify_values = parse_kv_line(rewritten_truncated_verify.output_line);
    require(rewritten_truncated_verify.status == 0,
            "rewritten truncated WAL case should verify cleanly");
    require(rewritten_truncated_inspect_values.at("wal_truncated") == "0",
            "rewritten truncated WAL case should clear the truncation signal");
    require(rewritten_truncated_inspect_values.at("rewrite_recommended") == "0",
    ...
    const auto legacy_inspect_values = parse_kv_line(legacy_inspect.output_line);
    const auto legacy_verify = capture_verify_format(legacy_db_path);
    const auto legacy_verify_values = parse_kv_line(legacy_verify.output_line);
    require(legacy_verify.status == 2, "legacy compatibility matrix case should require rewrite");
    require(legacy_inspect_values.at("snapshot_version") == "1", "legacy case should report snapshot v1");
    require(legacy_inspect_values.at("rewrite_recommended") == "1", "legacy case should recommend rewrite");
    require(legacy_verify_values.at("verify_reason") == "migration_required",
            "legacy case should expose a migration-required reason");

    require(rewrite_format_analysis(legacy_db_path) == 0, "legacy compatibility matrix case should rewrite successfully");
    const auto rewritten_inspect = capture_inspect_format(legacy_db_path);
    const auto rewritten_inspect_values = parse_kv_line(rewritten_inspect.output_line);
    const auto rewritten_verify = capture_verify_format(legacy_db_path);
    const auto rewritten_verify_values = parse_kv_line(rewritten_verify.output_line);
    require(rewritten_verify.status == 0, "rewritten legacy case should verify cleanly");
    require(rewritten_inspect_values.at("snapshot_version") == "2", "rewritten legacy case should upgrade to snapshot v2");
    require(rewritten_inspect_values.at("rewrite_recommended") == "0", "rewritten legacy case should stop recommending rewrite");
    ...
    std::cout << "case=current_v2"
              << " snapshot_version=" << current_inspect_values.at("snapshot_version")
              << " wal_version=" << value_or(current_inspect_values, "wal_version", "empty")
              << " verify_status=" << current_verify.status
              << " verify_reason=" << current_verify_values.at("verify_reason")
              << " rewrite_recommended=" << current_inspect_values.at("rewrite_recommended")
              << " current_verify_output_bytes=" << current_verify.output_line.size()
              << '\n';
    std::cout << "case=legacy_v1"
              << " snapshot_version=" << legacy_inspect_values.at("snapshot_version")
              << " wal_version=" << value_or(legacy_inspect_values, "wal_version", "missing")
              << " verify_status=" << legacy_verify.status
              << " verify_reason=" << legacy_verify_values.at("verify_reason")
              << " rewrite_recommended=" << legacy_inspect_values.at("rewrite_recommended")
              << " legacy_verify_output_bytes=" << legacy_verify.output_line.size()
              << '\n';
    std::cout << "case=legacy_v1_after_rewrite"
              << " snapshot_version=" << rewritten_inspect_values.at("snapshot_version")
              << " wal_exists=" << rewritten_inspect_values.at("wal_exists")
              << " verify_status=" << rewritten_verify.status
              << " verify_reason=" << rewritten_verify_values.at("verify_reason")
              << " rewrite_recommended=" << rewritten_inspect_values.at("rewrite_recommended")
              << " rewritten_verify_output_bytes=" << rewritten_verify.output_line.size()
              << '\n';
    std::cout << "case=current_v2_live_wal"
              << " snapshot_version=" << live_wal_inspect_values.at("snapshot_version")
              << " wal_version=" << value_or(live_wal_inspect_values, "wal_version", "missing")
              << " wal_records=" << live_wal_inspect_values.at("wal_records")
              << " verify_status=" << live_wal_verify.status
              << " verify_reason=" << live_wal_verify_values.at("verify_reason")
              << " rewrite_recommended=" << live_wal_inspect_values.at("rewrite_recommended")
              << " live_wal_verify_output_bytes=" << live_wal_verify.output_line.size()
              << '\n';
    std::cout << "case=current_v2_truncated_wal"
              << " snapshot_version=" << truncated_inspect_values.at("snapshot_version")
              << " wal_truncated=" << truncated_inspect_values.at("wal_truncated")
              << " verify_status=" << truncated_verify.status
              << " verify_reason=" << truncated_verify_values.at("verify_reason")
              << " rewrite_recommended=" << truncated_inspect_values.at("rewrite_recommended")
              << " truncated_verify_output_bytes=" << truncated_verify.output_line.size()
              << '\n';
    std::cout << "case=current_v2_truncated_wal_after_rewrite"
              << " snapshot_version=" << rewritten_truncated_inspect_values.at("snapshot_version")
              << " wal_truncated=" << rewritten_truncated_inspect_values.at("wal_truncated")
              << " verify_status=" << rewritten_truncated_verify.status
              << " verify_reason=" << rewritten_truncated_verify_values.at("verify_reason")
              << " rewrite_recommended=" << rewritten_truncated_inspect_values.at("rewrite_recommended")
              << " rewritten_truncated_verify_output_bytes=" << rewritten_truncated_verify.output_line.size()
              << '\n';
    return 0;
}
